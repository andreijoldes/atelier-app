...
const icons = {
  dashboard: (
    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-[11px] font-semibold leading-tight ${isActive ? 'text-primary' : ''}`}>{item.label}</span>
              </Link>
            );
          })}
        </div>
