...
// Pe mobile: bară de jos (bottom navigation) — 5 tab-uri
// Pe desktop: sidebar lateral stâng
// Meniu: Dashboard, Clienți, Programări, Devize, Setări
...
const navItems = [
  ...
  { href: '/setari', label: 'Setări', icon: icons.setari },
];
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-0.5 py-1.5 px-2 rounded-xl transition-all duration-200 min-w-[56px]
                  ${active
                    ? 'text-primary'
                    : 'text-gray-400 hover:text-primary active:scale-95'
                  }`}
              >
                <span className={`[&>svg]:w-[22px] [&>svg]:h-[22px] transition-transform duration-200 ${active ? 'scale-110' : ''}`}>{item.icon}</span>
                <span className={`text-[10px] font-bold leading-tight ${active ? 'text-primary' : ''}`}>{item.label}</span>
